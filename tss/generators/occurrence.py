"""tss.generators.occurrence — χ 闭合状态机与最小组织记录（P2-A 核心）。
...
class OccurrenceClosure:
    ...
    def update(self, value: float, t_step: int, phys_support: bool = True) -> Optional[Occurrence]:
        ...
        默认 True——不传该参数的既有调用点在**单 epoch 场景**内行为不变
        （持续 True 意味着只在第一次上升沿开一次 epoch，此后
        `_epoch_consumed` 逻辑等效于原来的"ACTIVE 阶段不重复触发"行为）。
        **精确化（2026-09-08，外部评判《TSS (3) 清单》§2）**：该"行为
        不变"不适用于重整后再触发的场景——rearm 完成后若要产生第二次
        occurrence，必须有新的 False→True 支撑上升沿开启新 epoch；全程
        默认 True 的调用方第二次越阈会被 token 门控正确拒绝（这正是旧
        exp_P2A1b_3 判据4 exit 1 的原因，属实验语义过时而非状态机缺陷，
        负对照见该脚本判据6）。
