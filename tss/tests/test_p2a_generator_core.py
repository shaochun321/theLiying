...
#2→反馈）已收敛的核心裁定：
  ...
  - 十神经元结构已存在（`circuit/variant_adapter.py:_init_quantum_thermal_
    pathways`），本轮任务是抽取（`tss.generators.wrap_base_generator`），
    不是从零重建；
  - 发生检测需要正式的触发—退出—重整（trigger/exit/rearm）三边界闭合合同
    （`tss.generators.OccurrenceClosure`），不能只记首次上升沿；
...
from tss.generators.base_generator import (
    DRIVE_MODE_MANUAL_CALIBRATION, DRIVE_MODE_WORLD_COUPLED,
)
from tss.relations import FROZEN_THERMAL_SITES, snapshot_weights, check_frozen_weights_against
