...
执行顺序：T-072（多周期）→ 接口标定 → 半规管

| ID | 任务描述 | 优先级 | 依赖 | 来源文档 |
|:---|:---|:---|:---|:---|
| T-077 | ~~Motor链诊断~~（DEG-006：Motor正常，T-073度量bug。已于T-077诊断中证伪并存档） | ❌取消 | — | DEG-006 |
| T-072 | 热源消失多周期实验（200k，三周期）：跨周期权重是否累积？第二次Foraging更快？ | P0 | T-039b PASS | T070_T071_T072_实验方案.md |
| T-074 | 接口参数标定：muscle.gain + ThermalMouth.eta（联立，Motor链已确认正常）| P0-P1 | T-073 | 方案评判反馈报告_2026-07-07.md §接口标定 |
...
| ID | 任务描述 | 完成日期 | Commit |
|:---|:---|:---|:---|
| T-039b | 诈胡核查重跑（P0后全冻结STDP，50k）：3/3 PASS；DR5%=44.2%≤55%（随机基线）；w_ccw漂移=0.0004；P0不植入硬连线方向性 | 2026-07-07 | — |
