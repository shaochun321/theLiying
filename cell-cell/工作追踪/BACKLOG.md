...
执行顺序：T-073（P0验证）→ T-039重跑（诈胡核查）→ T-072（多周期）→ 接口标定 → 半规管

| ID | 任务描述 | 优先级 | 依赖 | 来源文档 |
|:---|:---|:---|:---|:---|
| T-073 | P0验证（50k）：fill饱和后w_ccw保留率>80%？DA_ema是否按预期衰减？对比P0前T-067轨迹 | P0 | — | 方案评判反馈报告_2026-07-07.md |
...
| T-074 | 接口参数标定：muscle.gain + ThermalMouth.eta（联立，T-073短程中顺带测量Motor激活幅度） | P0-P1 | T-073 | 方案评判反馈报告_2026-07-07.md §接口标定 |
