...
执行顺序：T-073（P0修复验证）→ T-072（多周期，含P0）→ T-071（参考）→ T-065（可选）

| ID | 任务描述 | 优先级 | 依赖 | 来源文档 |
|:---|:---|:---|:---|:---|
| T-073 | P0修复（DA慢积分EMA τ=5000）实装 + 50k权重保留验证：fill饱和后观察w_ccw衰减速率，对比修复前 | P0 | — | STDP架构解剖报告_2026-07-07.md |
| T-072 | 热源消失多周期实验：三周期200k步，测试STDP跨周期学习累积；需在T-073修复后执行 | P0 | T-073 | T070_T071_T072_实验方案.md |
| T-071 | STDP断路诊断：各节点电流采样，定位phasic→spinal→yaw→Motor断路点（降级为参考实验，断路B是增益问题非根因） | P1 | — | T070_T071_T072_实验方案.md |
| T-065 | 物理驱逐实验（step 100k 热源瞬移，可选）| P2 | — | 整合更新后的执行方案.md |
