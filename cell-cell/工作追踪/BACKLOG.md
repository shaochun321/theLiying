...
|:---|:---|:---|:---|:---|
| Posterior-0 | **执行完成（410eeae），⚠阻塞于 RULING-P0-1（RULING_EXPOSURE）裁定**：六门全 PASS → **A_CONDITIONAL**（POSTERIOR_OPERATOR_CANDIDATE=YES，operator_class=THRESHOLD_READOUT_GATING——Δ 残尾抬工作点过 DEG-019 阈值、门控历史残余可见性）。NF-P1：膜载体分量严格加法（w_shift_vm=3e-15，𝔅=id on Z-primary），交互全在读出投影 H_W 面（I_W=1.65e-2/时序依赖 6.71×/block+transplant 因果闭合/hold6 一次过）。**待裁**：若"载体分量改变"为支配判据 → 预登记降格 C+THRESHOLD_READOUT_INTERACTION 正发现。详见 research/posterior_v0/POSTERIOR0_FINAL_RULING.md | 2026-09-21 | 六门终裁完毕；**我方读法对照与建议已出（2026-09-22，交叉比对/Posterior-0终裁读法对照与建议_2026-09-22.md：主建议=维持 A_CONDITIONAL+NF-P1 升为 Posterior-1 前置 gate；次优=收窄宣称名；不建议降格 C——D-1 语义失真[C 原义=I_W=0，本轮 I_W=1.65e-2]/D-2 载体加法性是纯 RC+frozen 架构必然，该裁定实为路线级否定）**；裁定点保持 OPEN | 等外部反馈文档裁定 RULING-P0-1：(i)维持 A→Posterior-1 算子族提取（范围限 readout-gating 类）/(ii)维持字母+收窄宣称名/(iii)降格 C→"载体级基质何在"（须同时裁定是否解除 §5/§35 塑性禁令，否则该分支无合法元件可开工） |
