...
执行顺序：T-077（Motor诊断）→ T-039b（诈胡核查）→ T-072（多周期）→ 接口标定 → 半规管

| ID | 任务描述 | 优先级 | 依赖 | 来源文档 |
|:---|:---|:---|:---|:---|
| T-077 | Motor链诊断：Col→Motor束激活值/axis_acts/motor_decision输出。Motor_x全程=0，需确认信号断点 | P0 | — | T073_P0验证报告_2026-07-07.md §四 |
| T-039b | 诈胡核查重跑（全冻结STDP，50k）：区分P0/P1改善来自增益还是真实学习 | P0 | T-073 PASS | 方案评判反馈报告_2026-07-07.md |
| T-072 | 热源消失多周期实验（200k，三周期）：跨周期权重是否累积？第二次Foraging更快？ | P0 | T-073 | T070_T071_T072_实验方案.md |
| T-074 | 接口参数标定：muscle.gain + ThermalMouth.eta（联立，依赖T-077 Motor链修复） | P0-P1 | T-077 | 方案评判反馈报告_2026-07-07.md §接口标定 |
| T-075 | 接口参数标定：Body.friction + NOCI_DT_GAIN | P1 | T-074 | 物理接口规范_2026-07-07.md |
| T-071 | STDP断路诊断（降级参考实验）：各节点电流比，H-B假说验证 | P1 | T-039b | T070_T071_T072_实验方案.md |
| T-076 | 半规管信号注入（阶段0+0.5）：Cupula高通滤波 + ANGULAR_GAIN实测校准 | P1 | T-039b PASS | 半规管补全方案整合更新版.1.0.md |
| T-065 | 物理驱逐实验（step 100k热源瞬移，可选）| P2 | — | — |
