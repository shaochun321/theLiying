...
| T-067 | 交叉接线翻转实验：post-init注入翻转束（软断开原D1束w→0.001），实验组200k；判定 DR5>45% 且 w_correct>w_swap；**前置**：读 variant_adapter.py（phasic神经元属性名）+ bundle.py（LTD是否依赖DA）| P1 | — | 防诈胡补全方案_评判与整合_2026-07-06.md |
| T-065 | 物理驱逐实验（step 100k 热源瞬移，可选）| P1 | — | 整合更新后的执行方案.md |
