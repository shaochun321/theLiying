...
|:---|:---|:---|:---|:---|
| MFS0-E0 | **Step 0/A/B/C 全部 PASS（2026-09-23），等用户签收后可翻 READY_FOR_MAIN_FACTORIAL=YES**。修订方案（34 节）R-1~R-4 已裁定并执行：R-1 M_SOURCE=H_tau_delta / R-2 TIER=2（含 LIMITATION 登记）/ R-3 POSTERIOR0_TERMINAL_C_FROZEN / R-4 w₀=0.90。**四门实测**：MFS0-M2 PASS（τ_e=0.300000s 误差 0.0000%，e_peak W1/W0=3.305/0.000e+00）；SEMANTIC_ERASURE_AUDIT PASS（10 污染点拒绝+M链 0 命中）；MFS0-M3 PASS（τ_M=0.600000s 误差 0.0000%，b↑=1）；五问 PASS（同 w 重跑 RMS=0.000e+00 逐位，零输入 5000 步 w 漂移 0）；**READOUT_FEASIBILITY PASS**（sham-repeat RMS 精确 0 ⇒ ε_Q 冻结 1e-6）。**关键新发现**：WRITE_DT_CORRECTION——Memristor.update 缺 dt 因子（自称 Strukov 型但 dw=0.5·i·(pre−post) 无 Δt），dt=0.001 下放大 1000×，单步 dw≈−0.5 一步撞 w_min；调用侧补 dt，建议下轮登记新 DEG。**我方 E-4 数值自纠**：w₀ 0.109→0.90 的相对效应改善是 8.18× 而非"三个数量级"（990 属 w→1.0）。预期 Δw 跨两数量级且强依赖 later 时点（2.22e-3 ~ 2.25e-1），Step D 选点有充分依据。详见 工作报告/MFS0-E0轮工作报告_Step0ABC_2026-09-23.md | 2026-09-23 | Step 0/A/B/C 完成；回归 21/21+contracts 13/15+vp+fast43 | Step D~H + hold6 + 六门终裁（下一批）；先落 ε_sel/S_min/predicted_Iz 三阈值（§17/§16 要求 factorial 前预注册） |
| （备注·已消费） | **MFS0-E0 方案评判（2026-09-23，ADOPT with amendments，见 交叉比对/MFS0-E0方案评判_2026-09-23.md）**。方案定位=对我方 Posterior-0 D-2 论点的正面回应（元件级设计轮）：Past→e（eligibility 电容）+ Later→M（调制）⇒ Δw（memristive 持久态）→ Future；两阶段（Phase A 物理绑定 / Phase B 持久沉积），六门 MFS0-M1~M6，六终态 A~F。**三项核心勘误**：E-1 §10 的 I_z=(w11−w10)−(w01−w00) 在乘性实现下是恒等式（w01=w10=w00）⇒ 不可证伪，主判据须改 NC3 选择性比值 Δw(NEAR)/Δw(TC)；E-2 物理乘法三档未指定（裸 Python×/双 FET 硬截零[theta_comparator 先例，已有 2026-09-06 用户裁定]/负载线自洽[仅 rail_latch]），§3 禁 if 却允许"ẇ∝e·M 最简近似"自相矛盾；E-4 M5 会撞 LIM-RPREC-READOUT-001（G(w)=1/(10−9.9w) 双曲，低 w 端 ΔG/G=0.117%≪Δw/w=0.97% 且 N-不变）⇒ 可事前解析预见的终态 E 风险，须先选高 w 工作点。**好消息**：时点可达性非硬阻塞（site23 已录窗 3542/4159/4577 + P0 新录 5402/6344 全晚于 W=rc_main），new_g0 预算可声明=0 | 2026-09-23 | 方案评判完毕（E-1~E-12 + R-1~R-4） | 等待 R-1（M(t) 驱动源四选一：ν/Ξ残差/CPG/H_τ，建议 H_τ 或 CPG）+ R-2（乘法档位，建议档2）+ R-3（与 RULING-P0-1 关系，建议保持 OPEN 待 MFS0 完成后合并裁定）+ R-4（初始 w 工作点偏离默认 0.1 改高 w 端） |
