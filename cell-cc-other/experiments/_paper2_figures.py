...
ax.errorbar(n_vals, means, yerr=stds, fmt='o', markersize=7,
            ...
            capsize=4, capthick=1.5, label='Measured $\\mathbb{E}[L]$',
            zorder=5)

# Annotate the n=3 rebound
ax.annotate('Rebound\n(n > 2.5)',
            xy=(3.0, 5.35), xytext=(3.3, 8),
            fontsize=9, color='#8e44ad',
            arrowprops=dict(arrowstyle='->', color='#8e44ad'),
            ha='center')

# Region labels
ax.axvspan(0.3, 1.0, alpha=0.05, color='green')
ax.axvspan(2.0, 3.5, alpha=0.05, color='red')
ax.text(0.65, 13.5, 'Theory-\nagreement\nzone', fontsize=8, ha='center',
        color='green', alpha=0.7)
ax.text(2.75, 2.0, 'Rebound\nzone', fontsize=8, ha='center',
        color='red', alpha=0.7)

ax.set_xlabel('Decay exponent $n$')
ax.set_ylabel('Mean equilibrium distance $\\mathbb{E}[L]$')
ax.set_xlim(0.2, 3.5)
ax.set_ylim(0, 16)
...
plt.close()
print("-> fig5_EL_vs_n.pdf")

# ─── Figure 6: Residual analysis ───
fig2, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 4))

# Panel a: ratio measured/predicted
L_pred = [(n * A * math.sqrt(N_p) / sigma) ** (1.0 / (n + 1)) for n in n_vals]
ratios = [m / p for m, p in zip(means, L_pred)]
ax1.bar(range(len(n_vals)), ratios, color=['#27ae60' if 0.85 < r < 1.15 else '#e74c3c' for r in ratios],
        width=0.6, edgecolor='#333', linewidth=0.5)
ax1.set_xticks(range(len(n_vals)))
ax1.set_xticklabels([f'{n:.1f}' for n in n_vals], fontsize=9)
ax1.set_xlabel('Decay exponent $n$')
ax1.set_ylabel('$\\mathbb{E}[L] / L^*_{theory}$')
