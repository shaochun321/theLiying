...
class GovernanceLedger:
    ...
    def _effective_activity(n) -> float:
        ...
        HairCells: use release_rate (Ca²⁺ output).
        """
        nid = n.config.neuron_id
        # HairCells output via release_rate, not activation
        if nid.startswith('hc_') and hasattr(n, 'release_rate'):
            return abs(n.release_rate)
