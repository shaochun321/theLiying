...
Checks:
    ...
    F6. Information heat death: K_ema ≈ 0 AND Σ|ξ| ≈ 0 → prediction engine dead
...
class Fuse:
    ...
    k_ema_min: float = 1e-6      # F6: shadow K_ema floor
    xin_abs_min: float = 0.01    # F6: global Σ|ξ| floor
    f6_confirm_steps: int = 5000 # F6: sustained dead ticks before trip

    # State
    enabled: bool = True
    trip_count: int = 0
    _f6_dead_counter: int = 0    # F6: consecutive dead ticks
    ...
    def check(self, circuit, dt: float) -> List[str]:
        ...
        # ── F6: Information heat death ──
        # Shadow layer prediction engine has gone silent.
        # K_ema ≈ 0 means shadow sees no surprise in sensory input.
        # Σ|ξ| ≈ 0 means no bundle has prediction error.
        # Both together = the organism is informationally dead.
        # More dangerous than divergence: all conservation laws hold,
        # but the system has ceased to function as a living entity.
        shadow = getattr(circuit, 'shadow_sandbox', None)
        if shadow is not None and shadow._initialized:
            k_ema = shadow._k_ema
            xin_abs_sum = sum(
                abs(b.config.xin_tension) for b in bundles
            )
            if k_ema < self.k_ema_min and xin_abs_sum < self.xin_abs_min:
                self._f6_dead_counter += 1
            else:
                self._f6_dead_counter = max(0, self._f6_dead_counter - 1)

            if self._f6_dead_counter >= self.f6_confirm_steps:
                violations.append(
                    f"F6 Information heat death: "
                    f"K_ema={k_ema:.2e}, Σ|ξ|={xin_abs_sum:.4f}, "
                    f"sustained for {self._f6_dead_counter} steps. "
                    f"Prediction engine is dead."
                )
