# v40.9 Walkthrough — CPG Bio-Substrate + Pipeline Integration

## Problem Statement

v40.8 suffered **thermal death**: after tick ~211, the system froze (T→0.048, PRP=0, convergence→0, inter-layer bundles frozen at 9). No structural changes occurred for 1800+ ticks.

## Solution: Three-Layer Fix

### 1. CPG Half-Center Oscillator (`_cpg_step()`)

Added a Central Pattern Generator using reciprocal inhibition:

```
cpg_fast_a ⟷ cpg_fast_b    (theta ~8 tick period)
cpg_slow_a ⟷ cpg_slow_b    (cardiac ~20 tick period)
         ↓
  visc_rhythm, visc_baseline  (encoding layer visceral zone)
         ↓
  encoding layer baseline drive
```

**Mechanism**: Tonic drive (I_NaP proxy) + reciprocal inhibition + adaptation-driven swap.

### 2. CPG-Driven Ghost Resurrection

**Bug found**: Inter-layer operations (contraction, pruning, ghost management) were inside the per-layer `for layer` loop — running 4× per tick. Resurrected bundles were immediately re-contracted.

**Fix**: Moved inter-layer operations outside the per-layer loop. Added resurrection logic:
- Condition: source neurons active (calcium > 0.001) AND ghost aged > 100 ticks
- Resurrected with strength 0.2 (above 0.1 contraction threshold)

### 3. CPG-Modulated Convergence Decay

**Problem**: `decay = 0.99` → `0.99^2000 = 2e-9` → all convergence nodes die.

**Fix**: When visceral zone is active, decay = 0.997 (slower). This mirrors sleep-dependent memory consolidation.
...
```
┌──────────────────────────────────────────────────┐
│  signal_entropy (7 neurons)     — external input │
├──────────────────────────────────────────────────┤
│  cpg (4 neurons)                — bio-substrate  │
│  cpg_fast_a ⟷ cpg_fast_b      (theta)          │
│  cpg_slow_a ⟷ cpg_slow_b      (cardiac)        │
├──────────────────────────────────────────────────┤
│  encoding (7 z_t + 8 cx_ + visc_)  — encoding   │
│  + visc_rhythm, visc_baseline      (NEW)         │
├──────────────────────────────────────────────────┤
│  column (7 neurons)             — column layer   │
└──────────────────────────────────────────────────┘

Pipeline DB ← v40_circuit_state_ledger (NEW)
JSON Report ← cpg_bio_substrate section (NEW)
```
...
class HebbianCircuit:
    ...
    # ─── v40.9: CPG Bio-Substrate Oscillator ───

    def _cpg_step(self):
        """Central Pattern Generator: half-center reciprocal inhibition.

        Generates self-sustaining oscillations via two neuron pairs:
          cpg_fast_a ←→ cpg_fast_b   (theta-like, ~8 tick period)
          cpg_slow_a ←→ cpg_slow_b   (cardio-like, ~20 tick period)

        Each pair uses the half-center model (Brown 1911, Marder & Bucher 2001):
          - When A > B: A suppresses B, A accumulates adaptation (fatigue)
          - Adaptation builds → A weakens → B released → roles swap

        DEGRADED: pacemaker neuron intrinsic bursting (I_h, I_NaP, T-type Ca²⁺)
        degraded_from = "CPG_half_center_reciprocal_inhibition"

        The CPG layer is a bottom-level structural input source with the same
        spatiotemporal measurement framework as signal_entropy. It is prepared
        for real signal source binding (compute metrics, hardware telemetry)
        through the standard inter-layer bundle interface.
        """
        cpg = self.layers.get("cpg")
        if cpg is None:
            return

        # ── Half-center pairs ──
        pairs = [
            ("cpg_fast_a", "cpg_fast_b", 0.12),   # fast pair: high adaptation
            ("cpg_slow_a", "cpg_slow_b", 0.05),    # slow pair: low adaptation
        ]

        for nid_a, nid_b, adapt_rate in pairs:
            na = cpg.neurons.get(nid_a)
            nb = cpg.neurons.get(nid_b)
            if na is None or nb is None:
                continue

            # Read adaptation state (fatigue accumulation)
            adapt_a = getattr(na, '_cpg_adaptation', 0.0)
            adapt_b = getattr(nb, '_cpg_adaptation', 0.0)

            # Effective activation = raw activation - adaptation
            eff_a = max(0.0, na.activation - adapt_a)
            eff_b = max(0.0, nb.activation - adapt_b)

            # Reciprocal inhibition: winner suppresses loser
            suppression = 0.5
            if eff_a > eff_b:
                # A is dominant → suppress B, A accumulates fatigue
                nb.activation = max(0.0, nb.activation - suppression * eff_a)
                na._cpg_adaptation = adapt_a + adapt_rate * eff_a
                # B recovers from fatigue
                nb._cpg_adaptation = max(0.0, adapt_b - adapt_rate * 0.5)
            else:
                # B is dominant → suppress A, B accumulates fatigue
                na.activation = max(0.0, na.activation - suppression * eff_b)
                nb._cpg_adaptation = adapt_b + adapt_rate * eff_b
                # A recovers from fatigue
                na._cpg_adaptation = max(0.0, adapt_a - adapt_rate * 0.5)

            # Tonic drive: intrinsic current that keeps the oscillation alive
            # DEGRADED: persistent sodium current (I_NaP) → proxy as constant input
            # degraded_from = "persistent_sodium_current_I_NaP"
            tonic_drive = 0.02
            na.activate(tonic_drive)
            nb.activate(tonic_drive)

            # Adaptation decay (slow recovery from fatigue)
            na._cpg_adaptation = getattr(na, '_cpg_adaptation', 0.0) * 0.95
            nb._cpg_adaptation = getattr(nb, '_cpg_adaptation', 0.0) * 0.95

        # Propagate CPG → encoding via inter-layer bundles
        # This happens automatically in transport(), but we also need
        # to ensure the visceral zone receives CPG output every tick
        # even when no external transport() call is made.
        for bundle in self.inter_layer_bundles:
            if not bundle.bundle_id.startswith("cpg_to_"):
                continue
            src_acts = [cpg.neurons[sid].activation
                        for sid in bundle.source_neuron_ids
                        if sid in cpg.neurons]
            if not src_acts:
                continue
            # Get post activations for NMDA
            post_acts = []
            for tid in bundle.target_neuron_ids:
                for lid, l in self.layers.items():
                    if tid in l.neurons:
                        post_acts.append(l.neurons[tid].activation)
                        break
                else:
                    post_acts.append(0.0)
            tgt_acts = bundle.propagate(src_acts, post_activations=post_acts)
            # Apply to target neurons in encoding
            for lid, l in self.layers.items():
                for j, tid in enumerate(bundle.target_neuron_ids):
                    if tid in l.neurons and j < len(tgt_acts):
                        l.neurons[tid].activate(tgt_acts[j])
    ...
    def maintain(self):
        ...
        v40.9: CPG step runs first — bio-substrate oscillation provides
        baseline drive before any decay/pruning decisions.
        """
        # v40.9: CPG oscillation — must run BEFORE decay
        self._cpg_step()
        ...
        for layer in self.layers.values():
            ...
            # Ghost decay: ghosts slowly fade but never reach zero (intra-layer)
            for g in layer._ghost_bundles:
                g["ghost_strength"] = max(0.0001,
                    g["ghost_strength"] * 0.99)  # 1% decay per tick

            # Intra-layer ghost resurrection (tension-based)
            revived = []
            still_ghosts = []
            for g in layer._ghost_bundles:
                # Check if ghost's target neurons have high unmet tension
        ...
        # ── Inter-layer operations (run ONCE per tick, after all layers) ──
        ...
        # Inter-layer ghost decay
        for g in self._inter_ghost_bundles:
            g["ghost_strength"] = max(0.0001,
                g["ghost_strength"] * 0.995)  # slower decay for inter-layer

        # v40.9: CPG-driven inter-layer ghost resurrection
        # Spontaneous activity (CPG) during rest reactivates dormant
        # synaptic pathways — similar to memory reactivation during sleep
        # (Diekelmann & Born 2010, Wilson & McNaughton 1994).
        # DEGRADED: sleep-dependent memory reactivation → CPG tonic drive proxy
        # degraded_from = "spontaneous_reactivation_during_rest"
        enc_layer = self.layers.get("encoding")
        visc_active = False
        if enc_layer:
            vr = enc_layer.neurons.get("visc_rhythm")
            vb = enc_layer.neurons.get("visc_baseline")
            visc_active = (vr is not None and vr.calcium > 0.0001) or \
                          (vb is not None and vb.calcium > 0.0001)

        if visc_active and self._inter_ghost_bundles:
            il_still_ghosts = []
            il_revived = []
            for g in self._inter_ghost_bundles:
                # Check if SOURCE neurons are active (signal to carry)
                src_ids = g.get("source_neuron_ids", [])
                src_calcium = 0.0
                for lid2, l2 in self.layers.items():
                    for sid in src_ids:
                        if sid in l2.neurons:
                            src_calcium += l2.neurons[sid].calcium
                # Resurrect condition: source neurons are active (have signal)
                # AND ghost has aged enough (not freshly pruned)
                age = self.tick - g.get("created_tick", 0)
                if src_calcium > 0.001 and age > 100:
                    new_b = MetaSynapticBundle(
                        bundle_id=g["bundle_id"] + "_cpg_revived",
                        source_neuron_ids=src_ids,
                        target_neuron_ids=g.get("target_neuron_ids", []),
                        bundle_strength=0.2,  # above 0.1 contraction threshold
                        bundle_inertia=0.5,
                    )
                    new_b.init_weights()
                    for i in range(len(new_b.weights)):
                        for j in range(len(new_b.weights[i])):
                            new_b.weights[i][j] = 0.05  # small initial weights
                    il_revived.append(new_b)
                    pruned.append(f"IL_CPG_REVIVED:{g['bundle_id']}")
                else:
                    il_still_ghosts.append(g)
            self._inter_ghost_bundles = il_still_ghosts
            self.inter_layer_bundles.extend(il_revived)
    ...
    def _update_convergence(self):
        ...
        # v40.9: CPG modulates convergence decay — spontaneous activity
        # slows memory trace decay (sleep-dependent consolidation proxy)
        # degraded_from = "sleep_dependent_memory_trace_maintenance"
        decay = self._convergence_decay  # default 0.99
        visc_r = enc.neurons.get("visc_rhythm")
        visc_b = enc.neurons.get("visc_baseline")
        if (visc_r and visc_r.calcium > 0.0001) or (visc_b and visc_b.calcium > 0.0001):
            decay = 0.997  # slower decay when CPG is active

        for a_key in list(self._convergence_matrix.keys()):
            for b_key in list(self._convergence_matrix[a_key].keys()):
                self._convergence_matrix[a_key][b_key] *= decay
...
## Results: v40.8 vs v40.9 @ 2000 ticks

| Metric | v40.8 (thermal death) | **v40.9 (alive)** |
|--------|:----:|:----:|
| Temperature | 0.048 (frozen) | **0.047** (stable) |
| PRP | **0.0000** | **0.0073** ✅ |
| Convergence nodes | **0** (all dead) | **9** ✅ |
| IL bundles | 9 (frozen) | **12-17** (cycling) ✅ |
| IL ghosts | 8 (frozen) | **6-8** (cycling) ✅ |
| Column occupancy | 0.0000 | **0.044** ✅ |
| cx_→column bundles | 0 | **1-2** ✅ |
| Dormant fruits | 0 | **8** ✅ |
| Free energy F | 35.1 | **41.1** |
| Discrimination | YES | **YES** |

### Temperature History

```
v40.8:  T = 0.081 → 0.048 → 0.048 → 0.048  (FROZEN DEAD)
v40.9:  T = 0.083 → 0.047 → 0.047 → 0.047  (STABLE ALIVE)
```

### Causal Chain (CPG → PRP)

```
CPG tonic drive → visc_rhythm/baseline alive (pre_trace=38.0)
→ ghost resurrection trigger (visc calcium > 0.0001)
→ cx_→column bundles resurrected (strength=0.2)
→ column calcium accumulates
→ column calcium > prp_threshold → PRP emission > 0
→ synaptic tagging & capture reactivated
→ structural plasticity maintained
```

---

## Pipeline Feedback Loop (NEW)

### DB Write-Back: `v40_circuit_state_ledger`

| Column | Value @500 tick |
|--------|:---:|
| circuit_temperature | 0.0466 |
| free_energy | 42.67 |
| neuron_count | 49 |
| alive_count | 46 |
| il_bundle_count | 13 |
| il_ghost_count | 6 |
| total_prp | 0.0001 |
| convergence_node_count | 16 |
| cpg_slow_activation | 0.0131 |
| visc_calcium | 3.24e-5 |

### JSON Report: `cpg_bio_substrate` section

```json
{
  "neurons": {"cpg_fast_a": {...}, "cpg_slow_a": {...}, ...},
  "visceral_zone": {"visc_rhythm": {...}, "visc_baseline": {...}},
  "transport_heat": 5.89e-05,
  "il_bundles_total": 13,
  "il_ghosts_total": 6
}
```
...
## Degraded Components (29 total, +2 new)

| New Proxy | Real Mechanism |
|-----------|---------------|
| `spontaneous_reactivation_during_rest` | Sleep-dependent synaptic reactivation (Diekelmann & Born 2010) |
| `sleep_dependent_memory_trace_maintenance` | Offline memory consolidation during spontaneous activity |
