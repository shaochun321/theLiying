...
不满足的字段必须是：
  (a) 被替换为动态代理，或
  (b) 被标记为 DEGRADED proxy + degraded_from 注释
```
...
| 字段 | 生成 | 衰减 | 耦合 | 代理来源 |
|------|------|------|------|---------|
| `activation` | activate() | decay() → resting | 驱动所有下游 | — |
| `resting_potential` | activation_ema 适应 | 向 0 衰减 | 决定 decay 目标 | ion_channel_redistribution |
| `potential` | |activation| 积累 | — (单调) | 触发 try_mature | — |
| `inertia` | 稳定状态 +0.001 | 波动状态 -0.001 | 分母: STDP/activate | perineuronal_net_formation |
| `pre/post_trace` | activate/post_activate | exp 衰减 | 驱动 STDP | — |
| `calcium` | |activation| 积分 | → target_rate 衰减 | 驱动 threshold/PRP/suppression | — |
| `threshold` | calcium > target 时升 | calcium < target 时降 | 门控 activation | — |
| `energy` | calcium 比例恢复 | transport_cost 消耗 | gate: is_alive() | mitochondrial_ATP |
| `stdp_ltp_boost` | 向 base 恢复 | STDP 使用疲劳 | 调制 STDP LTP | vesicle_pool_recycling |
| `prp_emission` | calcium > threshold | × 0.95 衰减 | 被 Spine 果实捕获 | protein_synthesis_diffusion |
| `prp_threshold` | 初始化 | 向 calcium×1.5 适应 | 门控 PRP | transcription_factor |
| `lat_suppression` | try_mature | field_strength ∝ calcium | 推 Spine threshold | basket_cell_interneuron |

### MetaSynapticBundle 字段

| 字段 | 生成 | 衰减 | 耦合 | 代理来源 |
|------|------|------|------|---------|
| `weights` | STDP LTP | STDP LTD | 决定 propagate() | — |
| `bundle_strength` | weights 均值 | pruning | circulation measure | — |
| `bundle_inertia` | — | STDP activity 降低 | 分母: STDP/propagate | — |
| `transport_cost` | propagate 计算 | 每 tick 重计算 | 消耗 source energy | axonal_ATP_consumption |
| `xin_tension` | accumulate_xin | fruit 激活后清零 | 调制 fruit trace 衰减 | — |
| `xin_dormant_fruit` | tension > 0.5 | trace 衰减 → expire | PRP 捕获 + 第三因子 | dopaminergic_modulation |

---

## 验证结果 (v40.7c)

| 指标 | v40.6 基线 | **v40.7c** | 变化 |
|------|:---:|:---:|:---:|
| avg cos | 0.329 | **0.194** | ↓ 41% |
| cos(movie,gratings) | 0.498 | **0.000005** | ≈ 正交 |
| cos(scenes,gratings) | 0.059 | **0.055** | 稳定 |
| threshold std | 0.001134 | **0.001134** | 保持 |
| alive | 35/35 | **35/35** | 保持 |
| PRP | 不存在 | **0.96 total** | 分化中 |
| fruit trace | 不存在 | **[0.748, 0.991]** | Xin 调制 |
...
axonal_transport_ATP_consumption                  → proportional drain (inter-layer)
dendritic_transport_ATP_consumption               → proportional drain (intra-layer)
