...
以下条目不属于语义硬编码（不违反 RULES.md 定义），但需要跟踪：

| 编号 | 发现日期 | 现象 | 影响 | 处理决策 |
|------|---------|------|------|---------|
| OBS-001 | 2026-07-05 | `satiety_neuron.activation` 偶发 >1.0（实测最高 1.148） | 生理上饱和度不应超过 1.0；可能导致 satiety 门控过强。低优先级，不影响热趋性实验 | 记录观察，不修复。远期可选软饱和钳位 `min(act, 1.0)`，但需评估对饱腹闭环的影响 |
| OBS-002 | 2026-07-05 | P1 脑干前庭侧向抑制（12条 bundle）三问 Q2/Q3 未解 | 缺少：哪些 aff_reg/aff_irr 神经元对需要互联；initial_weight/synapse_gain 无 BIO 依据 | 暂缓，等待用户补充 Q2（具体 neuron 对象名称）和 Q3（BIO 参数来源）后实施 |
