"""World 2.0 Phase 4 — 1M 步长程多热源导航验证

**背景**：Phase 3 完成三热源正三角形 500k 步短程验证。Phase 4 延长至 1M 步，
测试：
  1. 方向权重能否在多次接近-撤退循环中持续积累（DR1 时间尺度延长）
  2. body 能否在多个热源间自主切换（多源访问）
  3. 热趋性在长程（1M步）中是否退化（DR5 稳定性）
  4. 能量系统是否在长程中维持平衡（DR3 稳定性）
...
**body 起点**：d=20 from S1（同 Phase 3，在能量吸收范围内）

**验收标准**：
  DR1: |wR-wL| > 0.005 OR |wB-wF| > 0.005（至少一方向学习，@600k）
  DR2: dist_to_nearest reduction > 3.0 at 600k 步（长程接近要求更严格）
  DR3: fill > 0.05 at 800k 步（长程能量维持）
  DR5: grad·v > 60%（长程热趋性，要求提高）
  DR4: 访问至少 2 个不同热源（d < 15 at any point）
...
VISIT_THRESHOLD = 15.0  # d < 15 counts as "visiting" a source
...
print("=" * 80)
print("World 2.0 Phase 4 -- 1M steps equilateral triangle long-range navigation")
print("=" * 80)
...
for step in range(STEPS):
    ...
    # Track which sources the body visits (d < VISIT_THRESHOLD)
...
# Use @600k weights if available, else final
...
passed = n_pass >= 5  # 5/7 required (DR4 and DR_cycles are new, harder)
print(f"\n{'='*80}")
