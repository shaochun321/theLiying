"""nexus_v1.tests.test_r_prec_t2 — P2-C0：第二个同等级R1原语构造验证。

方案依据：document - 2026-08-01T222037.115.md（"先复用现有结构模板构造
第二条时间关系，不同时展开空间、尺度和账本系统"）。

T2使用 FROZEN_THERMAL_SITES["t2_chain"] 的第二条边：hub(28)↔nb2(23)，
与T1（31↔28）共享枢纽站点28，构成三点链 31↔28↔23。
...
from nexus_v1.relations.relation_occurrence import RelationFinalizer
from nexus_v1.relations.temporal_r_prec import RPrecCircuitT1
from nexus_v1.relations.temporal_r_prec_t2 import (
    RPrecCircuitT2, RELATION_TYPE_B_PREC_C_FAST,
)
from nexus_v1.relations.site_selection import FROZEN_THERMAL_SITES
