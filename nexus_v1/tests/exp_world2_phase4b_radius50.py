"""World 2.0 Phase 4b — 1M 步长程多热源导航验证（radius=50 修复版）

**背景**：Phase 4（radius=30）因能量危机在步骤 ~350k 时 fill 归零，体无法在热源间导航。
根本原因：三热源正三角形边长 52mm，热源间的 midpoint（d≈26）在 radius=30 时能量净负收益。

**能量平衡分析（修复依据）**：
  drain ≈ 0.003/step（vascular + basal）
  income = T_local × dt = 5.0 × (1 - d/r) × 0.001

  radius=30: breakeven at d < 12.4mm（d=26 midpoint: income=0.00133 << drain）→ 崩溃
  radius=50: breakeven at d < 20mm  （d=26 midpoint: income=0.00272 ≈ drain → 平衡）
             centroid（d=30from each): income = 3×5.0×(1-30/50)×0.001 = 0.006 >> drain → ✅

**修复**：radius: 30 → 50。其余参数与 Phase 4 完全相同。

**热源布局（正三角形，同 Phase 4）**：
  S1=[80,50,25]  +x 方向（右）
  S2=[35,76,25]  -x+y 方向（左-前，120°）
  S3=[35,24,25]  -x-y 方向（左-后，240°）
  边长：≈ 52mm，三角形质心 [50,50,25]

**body 起点**：[60,50,25]，d=20 from S1（与 Phase 4 相同）

**验收标准**（与 Phase 4 相同）：
  DR1: |wR-wL| > 0.005 OR |wB-wF| > 0.005（至少一方向学习，@600k）
  DR2: dist_to_nearest reduction > 3.0 at 600k 步
  DR3: fill > 0.05 at 800k 步（能量维持）
  DR4: 访问至少 2 个不同热源（d < 15）
  DR5: grad·v > 60%（热趋性）
  DR_cycles: approach_count >= 3（至少 3 次接近循环）
  DR_fill: fill > 0 全程
  NP1: thermo 束 ν EMA > 0
"""
...
# Phase 4b: radius 30 → 50 (energy balance fix)
src1 = HeatSource(position=[80.0, 50.0, 25.0], energy=10_000.0,
                  temperature=5.0, radius=50.0)
...
print("=" * 80)
print("World 2.0 Phase 4b -- 1M steps equilateral triangle (radius=50, energy fixed)")
print("=" * 80)
print(f"S1={src1.position}  S2={src2.position}  S3={src3.position}")
print(f"body={init_pos}  dist_nearest={init_dist:.2f}  radius={src1.radius}")
print()
...
for step in range(STEPS):
    ...
    # DR5: patch-gradient proxy (circuit-accessible signal only)
    pt = c._patch_temps
    patch_grad = [
