...
（∅/T1-only/T2-only/T1∧T2）改用**同一次真实物理驱动**（热源放站点28，
复用T-R2F-3已验证参数），只切换R2输入bundle的synapse_gain：
  ∅       : bundle_r1t1_to_trace/bundle_r1t2_to_trace都切断
...
def test_r2c_1_and_2_irreducibility_and_output_necessity():
    ...
    y_empty = _run_and_collect_y(_make_driven_circuit(cut_t1_to_r2=True, cut_t2_to_r2=True))
    ...
    kappa = [y_both[t] - y_t1[t] - y_t2[t] + y_empty[t] for t in range(N_STEPS)]
    K_R2 = _l2_norm(kappa)
    K_out_R2 = _l2_norm([y_both[t] - y_out_cut[t] for t in range(N_STEPS)])
    eps = _l2_norm(y_empty) * 3 + 1e-10

    print(f"\n  Y_∅ norm={_l2_norm(y_empty):.6f}, Y_T1 norm={_l2_norm(y_t1):.6f}, "
