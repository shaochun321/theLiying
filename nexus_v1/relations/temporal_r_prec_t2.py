"""nexus_v1.relations.temporal_r_prec_t2 — T2: 基础时间关系生成元 r≺（第二条边 28↔23）。
...
T1（temporal_r_prec.py）使用边 NB1(31) ↔ CENTER(28)，T2 使用边 CENTER(28) ↔ NB2(23)。
两条关系共享枢纽节点 28，构成三点链 31↔28↔23，与 t2_chain 定义完全吻合。

T2 与 T1 的关系：
  T1 检测 site_a(28) 先于 site_b(31) → rho_{A≺B}
  T2 检测 site_b(28) 先于 site_c(23) → rho_{B≺C}
  命名：T2 里的"b"=站点28，"c"=站点23（不与 T1 的"a"/"b"混淆；T2 在自己的命名空间里）。
