...
> [!IMPORTANT]
> **结构约束动态原则**：超边集合 $\mathcal{B}$ 由结构给定，不由影子层决定。影子层可以利用已有超边（通过度规收缩揭示哪些超边重要），但不能创建新的超边。影子层 = 探照灯，不是建筑工。

> [!WARNING]
> **DEGRADED 定义 — 以备日后突破**
>
> 以上"结构约束动态"原则对应集成电路（trace 在制造时蚀刻，信号不能创造新 trace）。但真实神经系统比集成电路更复杂：
>
...
>
> 在生物学中，**慢时间尺度的结构变化确实可以由动态驱动**——影子层的持续压力或许真的能"长出"新的超边（神经链路）。
>
> 当前定义将此能力**降级为不实现**，保持"结构固定 + 权重可变"的简化模型。
> 日后如需突破此限制，升级路径：影子层 Xin 持续高张力 + 果实激活 → 触发 LiquidMetalRouter 创建新 bundle（结构性变化），但须满足 Xin 守恒和能量守恒。
>
> `degraded_from = "dynamic_structural_plasticity"`
> `degraded_reason = "结构计算与物理计算的边界在生物系统中是模糊的，当前简化为清晰边界"`
...
>
> 降级原因：完整沙盒递归的收敛性尚未经数学验证（Open Question §7.4），且实施复杂度高。
>
> 当前保留下葬+衰减+共振作为最小可行影子层，日后按以下路径升级：
> 1. 加入双重度规（主度规 + 影子收缩度规）
> 2. 影子层运行自己的 $P^{(1)}/R^{(1)}/Xin^{(1)}$
> 3. 层级自由能 $\mathcal{K} = \sum_n F^{(n)}$
> 4. 运动势从 $\nabla_z \mathcal{K}$ 涌现
>
> `degraded_from = "hierarchical_variational_sandbox"`
